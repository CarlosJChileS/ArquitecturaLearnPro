...
const LessonViewer: React.FC = () => {
  ...
  const { execute: getLessonById } = useEdgeFunction('course', 'getLessonById', { requireSession: false });
  const { execute: getCourseById } = useEdgeFunction('course', 'getCourseById', { requireSession: false });
  const { execute: getCourseLessons } = useEdgeFunction('course', 'getCourseLessons', { requireSession: false });
  ...
  const loadLessonData = async () => {
    ...
    try {
      ...
      const { data: lessonsData, error: lessonsError } = await supabase
        .from('lessons')
        .select('id, title, description, content, video_url, duration_minutes, order_index, type')
      ...
      if (lessonsError) {
        ...
        const formattedLessons: Lesson[] = lessonsData.map(lesson => ({
          ...
          content_type: lesson.type as 'video' | 'text' | 'quiz',
          course_id: courseId
        }));
      ...
      if (user && lessonId !== 'no-lessons') {
        ...
        if (!progressError && progressData) {
          setProgress({
            ...
            time_spent_minutes: progressData.time_spent || 0,
  ...
  const handleProgressUpdate = async (progressPercentage: number, completed: boolean = false) => {
    ...
    try {
      // Use direct database update (skip edge function for now)
      ...
      const progressData = {
        ...
        time_spent: Math.floor(currentTime / 60)
        // Removed last_position - column doesn't exist
      };
          ...
          <div className="space-y-6">
            ...
            <Card>
              ...
              <CardContent>
                ...
                {/* Temporary button to complete all lessons for testing */}
                {!isCoursCompleted() && (
                  <Button 
                    onClick={async () => {
                      if (!user || !courseId || allLessons.length === 0) {
                        alert('❌ No se puede completar el curso');
                        return;
                      }
                      
                      console.log('🚀 Completing all lessons...');
                      const { supabase } = await import('@/integrations/supabase/client');
                      
                      // Mark all lessons as completed
                      const progressData = allLessons.map(lesson => ({
                        user_id: user.id,
                        lesson_id: lesson.id,
                        course_id: courseId,
                        is_completed: true,
                      ...
                      if (error) {
                        ...
                        alert('✅ Todas las lecciones completadas!');
                        setTimeout(() => loadCourseProgress(), 500);
                      }
                    }}
                    variant="outline"
                    size="sm"
