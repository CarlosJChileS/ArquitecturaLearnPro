...
const ExamPage: React.FC = () => {
  ...
  const loadExam = async () => {
    try {
      setLoading(true);
      const data = await examService.fetchExam(examId || '');
  ...
  const submitExamAnswers = async () => {
    ...
    try {
      ...
      setSubmitting(true);
      const timeTaken = exam.time_limit ? exam.duration_minutes * 60 - timeRemaining : undefined;
      const result = await examService.submitExamAttempt(exam as any, Object.values(answers), timeTaken);
