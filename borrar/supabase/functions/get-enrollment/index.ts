...
serve(async (req) => {
  ...
  try {
    ...
    // Get detailed course progress using optimized SQL function
    const { data: courseProgress, error: progressError } = await supabaseServiceClient
    ...
    if (progressError) {
      console.warn("Failed to get detailed course progress:", progressError);
    } else {
      logStep("Course progress retrieved", courseProgress);
    }
