...
-- 1. PRIMERO: Verificar qué foreign keys existen actualmente    -- courses → instructor_id → profiles(user_id) (solo si hay usuarios)
...
BEGIN    
    -- Eliminar foreign keys de course_enrollments
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'courses' AND column_name = 'instructor_id') THEN
        -- Buscar un usuario real existente para usar como instructor por defecto
        DECLARE
        ...
        BEGIN
            ...
            IF default_instructor_id IS NOT NULL THEN
                ...
                WHERE instructor_id NOT IN (SELECT user_id FROM profiles WHERE user_id IS NOT NULL);
            ELSE
                -- Si no hay usuarios, permitir instructor_id NULL temporalmente
...
BEGIN
    ...
    -- courses → instructor_id → profiles(user_id)
    ALTER TABLE courses 
    ADD CONSTRAINT courses_instructor_id_fkey 
    FOREIGN KEY (instructor_id) REFERENCES profiles(user_id) ON DELETE SET NULL;
    RAISE NOTICE '✓ courses.instructor_id → profiles.user_id');
